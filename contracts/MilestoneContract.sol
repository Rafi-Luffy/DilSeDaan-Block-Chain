...
contract MilestoneContract is ReentrancyGuard, Ownable {
    ...
    modifier milestoneExists(uint256 _milestoneId) {
        require(_milestoneId > 0 && _milestoneId <= _milestoneIdCounter, "Milestone doesn't exist");
        _;
    }

    constructor(address _donationContract) Ownable(msg.sender) {
    ...
    function createMilestone(
        ...
        _milestoneIdCounter++;
        uint256 newMilestoneId = _milestoneIdCounter;
    ...
    function getPendingMilestones() external view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= _milestoneIdCounter; i++) {
