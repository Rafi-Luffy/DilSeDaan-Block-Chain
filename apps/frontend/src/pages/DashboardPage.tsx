...
export function DashboardPage() {
  ...
  const userName = user?.name || 'Kind Heart';
        ...
        <div className="container mx-auto px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row items-center justify-between">
              <div>
                ...
                <p className="text-xl text-warm-charcoal-light">Welcome back, {userName}! Here's the beautiful story of your generosity.</p>
              </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.2 }} className="flex items-center justify-between p-4 bg-warm-cream/80 rounded-xl">
                    ...
                    <div className="text-right"><p className="font-bold text-lg text-warm-green">{formatCurrency(donation.amount)}</p><span className={`text-xs px-2 py-1 rounded-full font-bold ${ donation.status === 'Used' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }`}>{donation.status}</span></div>
                  </motion.div>
                ))}
              </div>
            </motion.div>
          ...
          <div className="space-y-8 lg:sticky lg:top-8">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="warm-card text-center bg-gradient-to-br from-warm-blue to-warm-green p-6 text-white shadow-handmade">
              <Award className="h-12 w-12 mx-auto mb-2 text-orange-300" />
              ...
              <Progress value={(userStats.impactScore / 1000) * 100} variant="handmade" className="bg-white/30 [&>div]:bg-orange-400" />
