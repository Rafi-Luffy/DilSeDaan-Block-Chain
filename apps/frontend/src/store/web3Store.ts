...
export interface Web3State {
  ...
  // Contract addresses with Polygon optimization
...
export const useWeb3Store = create<Web3State>()(
  persist(
    (set, get) => ({
      ...
      // Estimate transaction cost with enhanced type support
      estimateTransactionCost: async (type: TransactionType) => {
        try {
          const { gasPrice } = get()
          ...
          const gasEstimates = {
            ...
            verification: 75000,
            audit: 120000,
            campaign_creation: 200000,
            withdrawal: 60000
          }

          const estimatedGas = gasEstimates[type] || 50000
      ...
      connectWallet: async (walletType: WalletType = 'metamask') => {
        try {
          ...
          switch (walletType) {
            ...
            case 'walletconnect':
              ...
              throw new Error('WalletConnect not yet supported')
              
            case 'coinbase':
              // Future: Coinbase Wallet integration  
              throw new Error('Coinbase Wallet not yet supported')
          ...
          set({
            ...
            walletConnection,
            isPolygonNetwork,
            networkError: isPolygonNetwork ? null : 'Please switch to Polygon network for optimal performance and lower fees'
          })
          ...
          // Auto-suggest switching to Polygon for better experience
          if (!isPolygonNetwork) {
            console.log('Consider switching to Polygon for lower fees and faster transactions')
            // Auto-suggest switching to Polygon
            setTimeout(() => {
          ...
          // Enhanced event listeners with persistence
          provider.on('accountsChanged', (accounts: string[]) => {
            if (accounts.length === 0) {
              ...
              set({ 
                account: accounts[0],
                walletConnection: { ...get().walletConnection!, address: accounts[0] }
              })
          ...
          // Listen for chain changes with enhanced handling
          provider.on('chainChanged', (chainId: string) => {
            ...
            // Get optimized gas price for new network
            if (isPolygonNetwork) {
              get().getOptimizedGasPrice()
            }
          })

          // Listen for disconnect with cleanup
          ...
          set({
            networkError: error.message || 'Failed to connect wallet',
            isConnected: false
          })
          throw error
        }
      ...
      disconnectWallet: () => {
        ...
        if (typeof window !== 'undefined' && window.ethereum) {
          // Note: MetaMask doesn't provide removeAllListeners, but disconnection will clean up
          try {
            // Clean up any existing listeners - this is handled by MetaMask internally
            console.log('Disconnecting wallet and cleaning up listeners')
          } catch (error) {
            console.warn('Could not clean up wallet listeners:', error)
          }
        }
      ...
      // Update transaction status with enhanced support
      ...
      // Enhanced campaign creation with better error handling
      createCampaign: async (title: string, targetAmount: string, deadline: number, ipfsHash: string) => {
        try {
          const { account, donationContractAddress, isPolygonNetwork } = get()
          
          if (!account) throw new Error('Wallet not connected')
          if (!donationContractAddress) throw new Error('Contract not available')
          if (!isPolygonNetwork) {
          ...
          const tx = await contract.createCampaign(title, targetAmountWei, deadline, ipfsHash)
          
          get().addPendingTransaction(tx.hash)
          await tx.wait()
          get().updateTransactionStatus(tx.hash, 'completed')
      ...
      // Enhanced wallet reconnection
      reconnectWallet: async () => {
        try {
          const { isConnected, account } = get()
          if (isConnected && account) {
            await get().initializeWeb3()
          }
        } catch (error) {
      ...
      getTransactionHistory: () => {
        const { completedTransactions } = get()
        return completedTransactions.sort((a, b) => b.timestamp - a.timestamp)
      },

      // Clear transaction history
      clearTransactionHistory: () => {
        set({ completedTransactions: [], pendingTransactions: [] })
      },
    }),
    {
