    ...
    (set, get) => ({
      ...
      login: async (email: string, password: string) => {
        ...
        try {
          ...
          if (email === 'Indian_tax_dep.charity@gmail.com' && password === 'ServingIndia') {
      ...
      loginAsAdmin: async (username: string, password: string) => {
        ...
        try {
          ...
          const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/auth/login`, {
            ...
            body: JSON.stringify({ email: username === 'admin' ? 'Indian_tax_dep.charity@gmail.com' : username, password }),
          })
