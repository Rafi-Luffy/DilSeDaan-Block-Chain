...
import { validateDonation } from '../middleware/security';
...
router.get('/:id', protect, async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    if (req.user?.role !== 'admin' &&
        req.user?.id !== donation.donor._id.toString() &&
        req.user?.id !== (donation.campaign as any)?.creator?.toString()) {
...
// @access  Private
router.post('/', protect, validateDonation, async (req: AuthenticatedRequest, res, next) => {
  try {
