...
import { polygonCacheMiddleware } from '../middleware/cache';
import { blockchainPerformanceMiddleware } from '../middleware/performance';
...
// Enhanced Polygon configuration for better performance (using localhost for development)
const POLYGON_MAINNET_RPC = process.env.POLYGON_RPC_URL || 'http://localhost:8545';
const POLYGON_MUMBAI_RPC = process.env.POLYGON_MUMBAI_RPC_URL || 'http://localhost:8545';

// Use localhost for development, Polygon for production
...
// Smart contract addresses - Updated with deployed addresses
const CONTRACTS = {
  polygon: {
  ...
  localhost: {
    donation: process.env.DONATION_CONTRACT_ADDRESS || '0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9',
    milestone: process.env.MILESTONE_CONTRACT_ADDRESS || '0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9',
    audit: process.env.AUDIT_CONTRACT_ADDRESS || '0x5FC8d32690cc91D4c39d9d3abcBD16989F875707'
  }
};

// Initialize contracts with complete ABIs
const getContracts = (network: 'polygon' | 'mumbai' | 'localhost' = 'localhost') => {
  let provider;
  if (network === 'localhost') {
    provider = new ethers.JsonRpcProvider('http://localhost:8545', { name: 'localhost', chainId: 1337 });
  } else {
    provider = network === 'mumbai' ? mumbaiProvider : polygonProvider;
  }
